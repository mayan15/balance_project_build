...
class Elecrochemical_platform:
    ...
    def ep_obj_run(self, particle_num=64, para_num=28, max_iter=500, task_num=500, all_gather=1):
        ...
        # 开始辨识并输出结果
        log.logger.debug('Start param identify !')
        ...
        log.logger.debug(f'ep_obj_run()运行完毕，参数 辨识 完成 ')
    ...
    def update_param_bound(self, get_bound_flag=True):
        if self.battery_type in ['LFP', 'lfp']:
            ...
            self.cut_voltage_low_limit = 2.5
            self.cut_voltage_up_limit = 4.25
            # NCM
    ...
    def get_parameters(self, particle_num, para_num, max_iter, task_num, all_gather):
        ...
        log.logger.debug(f'self.loss_func : {self.loss_func}')
    ...
    def post_process_parameters(self, arr_2d, param_filter_flag=True):
        ...
        if param_filter_flag:
            ...
            # # 09-24 电化学意义的筛选
            # parameters = self.filter_electro_significance(parameters)
        return parameters
    ...
    def parameters_filter_func(self, parameters):
        ...
        parameters = parameters[(parameters >= -0.5).all(axis=1)].reset_index(drop=True)
        # print(f' == 11 == Iden parameters: {parameters}')
        
        parameters = parameters[(parameters["current_energy (KWh)"] > -0.5)]
        # print(f' == 22 == Iden parameters: {parameters}')
        
        # capacity_multi_coeff = 1.1
        capacity_multi_coeff = 1.0  # 20250730 修改 容量筛选系数
        ...
        log.logger.debug(f' == 44 == 参数筛选功能关闭')
        # print(f' == 44 == 参数筛选功能关闭')
        if self.loss_func == 'mae':   # loss 不可以太大
            parameters = parameters[parameters["loss"] <= 35] 
    ...
    # 09-24 电化学意义的筛选
    def filter_electro_significance(self, parameters, threshold=0.99):
        
        # parameters_copy = parameters.copy()
        parameters['eps_pos'] = parameters['epss_pos'] + parameters['epsl_pos']
        parameters['eps_neg'] = parameters['epss_neg'] + parameters['epsl_neg']
        
        parameters = parameters[parameters['eps_pos'] < threshold]
        parameters = parameters[parameters['eps_neg'] < threshold]
...
class Electrochemical_NCM:
    ...
    def __init__(self, soc, rated_capacity):
        ...
        self.parameters_dict = {'Area': {'val': 0.1027, 'up': 0.123, 'low': 0.0822},  # parameters for 5Ah
                                ...
                                'L_sep': {"val": 2.5e-05, "up": 9e-05, "low": 9e-06},
                                ...
                                'm_neg': {"val": 5e-05, "up": 1e-04, "low": 1e-5},
                                ...
                                'resist': {'val': 0.0001, 'up': 0.0099, 'low': 0.0},
    ...
    def get_parameters_range(self):  # 注意可能存在的bug 初始电压值非ocv值
        ...
        # soc: soc_list 0 - 100
        # print(f'self.soc:{self.soc}, self.rated_capacity:{self.rated_capacity}')
        ...
        self.parameters_dict['cs0_neg']['low'] = self.parameters_dict['csmax_neg']['val'] * max(self.soc[0] / 100 - 0.3, 0.01)  # + 0.045 # 0.9 - 0.05 delta sto = 0.05 / 5%
        self.parameters_dict['cs0_neg']['up'] = self.parameters_dict['csmax_neg']['val'] * min(self.soc[0] / 100 + 0.2, 1)  # +0.055
        self.parameters_dict['cs0_pos']['low'] = self.parameters_dict['csmax_pos']['val'] * max(1 - self.soc[0] / 100 - 0.2, 0.01)  # 0.25 - 0.84
        self.parameters_dict['cs0_pos']['up'] = self.parameters_dict['csmax_pos']['val'] * min(1 - self.soc[0] / 100 + 0.2, 0.99)
    ...
    def get_up_low_bound(self):
        ...
        log.logger.debug(f'辨识Area upbound:{upbound[0]}')
        log.logger.debug(f'辨识Area lowbound:{lowbound[0]}')
        ...
        log.logger.debug(f'up : {self.upbound}')
        log.logger.debug(f'low : {self.lowbound}')
        log.logger.debug(f'p0 : {self.p0}')
...
def concat_param_of_different_identify(df_param_this_time, param_iden_this_count):
    ...
    log.logger.debug(f'len of df_param_this_time: {len(df_param_this_time)}')   
...
def create_electro_obj(dufv_obj, sipp_obj, input_class, **kwargs):
    ...
    log.logger.debug(f'kwargs : {kwargs}')
...
def param_identify_then_filter(dufv_obj, sipp_obj, max_iter=500):
    ...
    while df_param_all_concat.shape[0] < sipp_obj.iden_result_len_threshold and count < sipp_obj.each_cell_inden_cnt_Max:
        ...
        log.logger.debug(f'ep_obj.time:{ep_obj.time}')
        log.logger.debug(f'ep_obj.current:{ep_obj.current}')
        log.logger.debug(f'ep_obj.voltage:{ep_obj.voltage}')
        log.logger.debug(f'ep_obj.temperature:{ep_obj.temperature}')
        log.logger.debug(f'ep_obj.soc:{ep_obj.soc}')
        log.logger.debug(f'ep_obj.battery_type:{ep_obj.battery_type}')
        log.logger.debug(f'ep_obj.so_file_str:{ep_obj.so_file_str}')
        log.logger.debug(f'ep_obj.rated_capacity:{ep_obj.rated_capacity}')
        ...
        df_param_all_concat = concat_param_of_different_identify(df_param_all_concat, params_inden)
        count += 1
        log.logger.debug(f'第 {dufv_obj.vol_name} 节电池 辨识了 {count} 次')
        # print(f'第 {dufv_obj.vol_name} 节电池 辨识了 {count} 次')
        
    # params_inden = params_inden[dufv_obj.vol_name].reset_index(drop=True) 
    params_inden = df_param_all_concat.copy().reset_index(drop=True) 
    params_up_low = pd.DataFrame(params_up_low).loc[['up', 'low']].copy()
    log.logger.debug(f'params_inden: {params_inden}')
    ...
    log.logger.debug(f"[After Select] parameters: {parameters[['loss', 'Area', 'L_pos']]}")
    # print(f"[After Select] parameters: {parameters[['loss', 'Area', 'L_pos']]}")
    # assert 0
