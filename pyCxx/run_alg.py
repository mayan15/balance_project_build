...
def alg_execution(csv_data_df, filename, log):
    alg_rlt_list = []
    ...
    for index, df_value in enumerate(csv_data_df):      
        ...
        report_folder_path = os.path.join(first_level_folder, filename[index])
        # 第三级文件夹
        ...
        data_clean_rlt, df_cleaned = data_clean_overview.run(df_value, picture_folder_path, pickle_folder_path)
        # alg_rlt_list.append(data_clean_rlt)
        
        if data_clean_rlt['ErrorCode'][0] == 0 and len(df_cleaned) > 0:
            print("开始计算plus")
            first_order.analyse_cell_plus_data(data_clean_rlt['out']['脉冲数据'][0], 1, 5, 150)
            print("结束计算plus")
